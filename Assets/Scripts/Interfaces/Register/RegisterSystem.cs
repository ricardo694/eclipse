...
IEnumerator PingGoogle()
{
    ...
    {
        string validationError = ValidateFields(username, email, password, confirmPassword);
        if (validationError != null)
        {
            if (uiRegister != null) uiRegister.OnRegisterFailed(validationError);
            return;
        }
    ...
    private string ValidateFields(string username, string email, string password, string confirmPassword)
{
    ...
    private IEnumerator RegisterCoroutine(string username, string email, string password)
    {
        ...
        using (UnityWebRequest authReq = new UnityWebRequest(authUrl, "POST"))
        {
            ...
            catch (Exception ex)
            {
                ...
                if (uiRegister != null) uiRegister.OnRegisterFailed(friendlyMessage);
                yield break;
            }
        }
        ...
        using (UnityWebRequest dbReq = new UnityWebRequest(dbUrl, "POST"))
        {
            ...
            try
            {
                ...
                if (uiRegister != null) uiRegister.OnRegisterSuccess();
            }
            catch (Exception ex)
            {
                Debug.LogError($" [Auth] Fallo al registrar: {ex.Message}");
